...
\section{Loading Microarray Data into R}\label{SC2-sect:analysis}
...
\noindent Table \ref{Cockell_t3} gives an overview of the packages used in this chapter. A text file of the commands used can be found at
\begin{center}
\url{https://github.com/csgillespie/illuminia-analysis/}
\end{center}
...
\bottomrule
\end{tabular}
\caption{Packages (and versions) used in this chapter.}\label{Cockell_t3}
\end{table}
...
<<echo=FALSE>>=
#library(GEOquery)
#loading Illumina data direct from GEO and ArrayExpress is not supported - so get the raw data & go from there as before
#getGEO gets (usually) pre-normalised/processed data
#getGEO('GSE26817')
#so get the supplementary files - if you're lucky, there will be a file that lumiR can understand
#getGEOSuppFiles('GSE26817')
#gunzip('GSE26817/GSE26817_non-normalized.txt.gz')
#geo_test = lumiR(fileName='GSE26817/GSE26817_non-normalized.txt')
#an ArrayExpress example - again working from RAW data
#library("ArrayExpress")
#mtab606 = getAE("E-MTAB-606", type = "full")
#ae_test = lumiR(mtab606$rawFiles)
#pheno_data <- read.table(mtab606$sdrf, header=T, sep='\t')
#in this case, the array labels don't match in the raw data vs the metadata

@

<<echo=FALSE, warning=FALSE,message=FALSE, cache=TRUE, results='hide'>>=
library("GEOquery")
library("lumi")
...
The data for this chapter can be downloaded (within R) directly from the Array Express website. We simply load the \texttt{ArrayExpress} and \texttt{lumi} packages
...
The \texttt{arrayQualityMetrics} package is a generic set of quality control routines that can be applied to many types of array data. It produces a report of quality metrics, which can be used to make assessments about overall array quality, and diagnose batch effects. To generate the report, we load the package
...
@
\noindent The \textit{array intensity distributions} (not shown) metric suggests that array eleven may also be an outlier, although in this case it's variation from the other arrays is less convincing, so we will retain the array in the analysis.
