...
The functional relationship of the genes in our lists can be studied more directly by using the \texttt{GOstats} package\cite{Falcon15012007} to find Gene Ontology\cite{Ashburner:2000ja} terms that are statistically over-represented among the genes in the lists. To begin, we load the package
...
@
\noindent Then we map our list significant probes to Entrez gene identifiers (as these are the identifiers GOstats uses to find GO terms)
<<message=FALSE, warning=FALSE, tidy=FALSE, cache=TRUE>>=
...
@
\noindent To test whether a particular probe is over-represented, we use a hyper-geometric test; this is provided by the \texttt{GOstats} package. We create an S4 object that contains all the necessary information
<<message=FALSE, warning=FALSE, tidy=FALSE, cache=TRUE>>=
...
@
\noindent The top five gene ontology identifiers are given in table \ref{Cockell_T2}.
...
\bottomrule
\end{tabular}
\caption{Top five gene ontology identifiers with a brief description.}\label{Cockell_T2}
\end{table}
